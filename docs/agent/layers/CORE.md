...
- **Needs a log line? That's `main/`'s job.** Return a result or throw; `main/` wraps with `createLogger`. See [MAIN.md](MAIN.md).
- **Needs a bundled JSON catalog?** Use `readBundledJson` from `core/bundledData.ts` — this is the one sanctioned exception, since it does a synchronous `fs.readFileSync` but stays side-effect-free in behavior (same file in, same data out). See [DATA.md](DATA.md) for catalog file conventions, schema, and registration.
...
`pnpm qa` (see [../QA.md](../QA.md)) runs the full suite — run targeted tests with `pnpm exec vitest run test/core/<file>.test.ts` while iterating.
...
The logic under test isn't pure — move the I/O-touching part to `main/` and keep only the transformation in `core/`. See [MAIN.md](MAIN.md).

### Catalog shape changed and tests broke everywhere

The catalog's JSON contract changed. Fix the catalog file and `catalog.ts` loader/types together — see [DATA.md](DATA.md) for the registration and schema-change checklist.
