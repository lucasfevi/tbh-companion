...
For tab chrome, tokens, and layout, also read [UX.md](UX.md) and `docs/STYLING.md`.
...
4. **Prefer deriving state during render** over `useEffect` + `setState` mirroring props — see [RENDERER-PERFORMANCE.md](RENDERER-PERFORMANCE.md).
...
- Derive during render, not effects → [RENDERER-PERFORMANCE.md](RENDERER-PERFORMANCE.md) § Re-renders
...
Read [RENDERER-PERFORMANCE.md](RENDERER-PERFORMANCE.md) when refactoring a hot path or reviewing a PR that touches list rendering, context, or bundle imports.
